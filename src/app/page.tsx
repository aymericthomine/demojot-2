      ...
      void (async () => {
        try {
          ...
          if (job.mode === "jelly") {
            await loadArt(
              job.theme,
              THEMES[job.theme].ladder.flatMap((rung) => (rung.art ? [rung.art] : [])),
            ).catch(() => {});
          }
          const dress = {
            invert: job.invert,
